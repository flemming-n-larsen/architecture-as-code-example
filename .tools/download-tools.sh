...
if [ "$JAVA_VERSION" -lt 11 ]; then
    echo -e "${RED}Error: Java 11+ is required (found Java $JAVA_VERSION)${NC}"
    exit 1
fi
...
if [ -n "$STRUCTURIZR_DSL_JAR" ]; then
    # Create relative symlink: .tools/structurizr-dsl.jar -> structurizr-cli/lib/structurizr-dsl-X.X.X.jar
...
echo "  java -jar ${STRUCTURIZR_DSL_JAR} --version"
